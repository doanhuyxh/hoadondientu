<h3 class="m-auto"> Quản lý khách hàng</h3>
...
<script>
    ...
    async function AddEdit(id) {
        if (id == 0) {
            ...
            $("#exampleModalLabel").text("Cập nhật khách hàng");

            let formData = new FormData();
            formData.append('id', id);
            const res = await fetch("/admin-get-customer-id", {
                method: "POST",
                body: formData
            })
    ...
    function Delete(id) {
        fetch("/admin-delete-customer?id=" + id)
            .then(es => es.json())
            .then(res => {
                console.log(res)
                if (res.status == 200) {
